...
void twai_receive_task(void *arg)
{
	...
	// Configure TWAI node
	ESP_LOGI(TAG, "CONFIG_TWAI_BITRATE=%d", CONFIG_TWAI_BITRATE);
	...
	while (1) {
		twai_frame_t frame;
		if (xQueueReceive(xQueueDevice, &frame, portMAX_DELAY) != pdPASS) {
		...
		if (ext == 0) {
			sprintf(work, "Standard ID: 0x%03"PRIx32"%*s", frame.header.id, 5, "");
		} else {
			sprintf(work, "Extended ID: 0x%08"PRIx32, frame.header.id);
		}
		strcpy((char *)cmdBuf.payload, work);
		sprintf(work, "  DLC: %d  Data: ", frame.header.dlc);
		strcat((char *)cmdBuf.payload, work);

		if (rtr == 0) {
			for (int i = 0; i < frame.header.dlc; i++) {
				sprintf(work, "0x%02x ", frame.buffer[i]);
				strcat((char *)cmdBuf.payload, work);
			}
		} else {
